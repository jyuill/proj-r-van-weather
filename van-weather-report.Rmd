...
vw_data %>% slice_max(order_by=Total.Precip, n=nrec) %>% select(Date, Total.Precip) %>%
  ggplot(aes(x=reorder(as.character(Date), Total.Precip), 
             y=Total.Precip, label=Total.Precip))+geom_col()+
...
vw_data %>% slice_min(order_by=get(metric), n=nrec) %>% select(Date, !!sym(metric)) %>%
  ggplot(aes(x=reorder(as.character(Date), -get(metric)), 
             y=get(metric), label=get(metric)))+geom_col()+
  geom_text(hjust=0, size=4, nudge_y=0.5)+
  coord_flip()+
...
chart.title <- glue("Coldest ",nrec," Days by Temperature")
vw_data %>% slice_max(order_by=get(metric), n=nrec) %>% select(Date, !!sym(metric)) %>%
  ggplot(aes(x=reorder(as.character(Date), get(metric)), 
             y=get(metric), label=get(metric)))+geom_col()+
  geom_text(hjust=0, size=4, nudge_y=0.5)+
  coord_flip()+
