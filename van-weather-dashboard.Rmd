...
vw.temp.mth.yr <- vw_data %>% 
  ...
  summarize(ave_temp=mean(Mean.Temp, na.rm=TRUE),
            ave_min_temp=mean(Max.Temp, na.rm=TRUE),
            ave_min_temp=mean(Min.Temp, na.rm=TRUE),
            min_temp=max(Max.Temp, na.rm=TRUE),
...
gauge(value=max(vw_data_rec$Max.Temp, na.rm=TRUE),
      min=d1,
      max=max(vw.temp.mth.yr$max_temp)+1,
      sectors=gaugeSectors(
        danger=c(s1[[1]],smax2),
        warning=c(w1[[1]],s1[[1]]),
        success=c(dmin1,w1[[1]])
        ),
      symbol='c'
      )
```
...
```{r}
print(paste0("From: ", min(vw_data_rec$Date)), row.names=FALSE)
print(paste0("To: ", max(vw_data_rec$Date)), row.names=FALSE)
```
